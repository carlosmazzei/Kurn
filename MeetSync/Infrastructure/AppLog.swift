...
//  Console.app / Xcode's console. Filter by subsystem `com.ainoteflow.app` or by
...
//  `MEETSYNC_LOG=0` environment variable to start with logging disabled. When
...
enum AppLog {
    private static let subsystem = "com.ainoteflow.app"

    /// Master switch for all app logging. Defaults to enabled unless
    /// `MEETSYNC_LOG` is set to "0"/"false" at launch.
    nonisolated(unsafe) static var isEnabled: Bool = {
        if let raw = ProcessInfo.processInfo.environment["MEETSYNC_LOG"] {
