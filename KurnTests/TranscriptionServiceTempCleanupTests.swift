...
struct TempFileCleanerTests {
    ...
    @Test func cleanupOrphanedTempFilesRemovesOldUploadBodies() async throws {
        try await tempFileTestLock.run {
    ...
    @Test func forceCleanupRemovesAllKnownFilesRegardlessOfAge() async throws {
        try await tempFileTestLock.run {
            let tmp = FileManager.default.temporaryDirectory
    ...
    @Test func forceCleanupRemovesAllUploadBodies() async throws {
        try await tempFileTestLock.run {
            let uploadDir = FileManager.default.temporaryDirectory
                .appendingPathComponent("WhisperUploadBodies", isDirectory: true)
            try FileManager.default.createDirectory(at: uploadDir, withIntermediateDirectories: true)
    ...
    @Test func reclaimableSpaceCountsEligibleFilesWithoutRemovingThem() async throws {
        try await tempFileTestLock.run {
