...
```bash
ffmpeg -f lavfi -i "sine=frequency=440:sample_rate=44100:duration=6.0" \
  -ac 1 -c:a aac -b:a 128k -movflags +faststart tone-44100-128kbps-6s.m4a

ffmpeg -f lavfi -i "sine=frequency=440:sample_rate=16000:duration=2.0" \
  -ac 1 -c:a aac -movflags +faststart tone-16000-2s.m4a
```

- `tone-44100-128kbps-6s.m4a` — the shape the recorder produced before the fixed
