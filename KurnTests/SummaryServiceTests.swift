...
struct SummaryServiceTests {
    ...
    ) -> SummaryService.TranscriptGroup {
        SummaryService.TranscriptGroup(
            offset: offset,
            segments: segments,
    ...
    @Test func assembleTranscriptTextFormatsTimestampSpeakerAndText() {
        ...
        let text = SummaryService.assembleTranscriptText(from: [group(segments)])
    ...
    @Test func assembleTranscriptTextFlattensMultipleGroupsInOrder() {
        ...
        let text = SummaryService.assembleTranscriptText(from: [group(groupOne), group(groupTwo)])
    ...
    @Test func assembleTranscriptTextShiftsLaterGroupsByOffset() {
        ...
        let groupOne = [TranscriptSegment(speakerLabel: "Speaker 1", startTime: 0, endTime: 1, text: "first")]
        let groupTwo = [TranscriptSegment(speakerLabel: "Speaker 1", startTime: 0, endTime: 1, text: "second")]
        let text = SummaryService.assembleTranscriptText(from: [group(groupOne), group(groupTwo, offset: 60)])
    ...
    @Test func assembleTranscriptTextIsEmptyForNoSegments() {
        ...
        #expect(SummaryService.assembleTranscriptText(from: [group([])]).isEmpty)
    }
    ...
    @Test func assembleTranscriptTextPrefixesLineContainingAHighlight() {
        ...
        let text = SummaryService.assembleTranscriptText(from: [group(segments, highlights: highlights)])
