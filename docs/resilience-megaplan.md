...
| H3    | PR 1 in progress       | `RecordingTrash` (branch `claude/plano-resiliencia-xe25b2`) makes meeting/recording deletion move-then-purge instead of delete-then-delete, with launch/foreground reconciliation for an interrupted delete. Still open: a general operation journal, quarantine for unmatched originals, migration collision handling, the unprotected `recordingsDirectoryURL` fallback, and typed authoritative JSON corruption. |
