...
| **H6**  | Active requests are origin-pinned, bounded, correlated and circuit-gated; background upload is disabled, while connection-cost controls remain incomplete                                          | Large cloud audio can still start on an expensive or constrained connection without an explicit transfer policy             | **P0**                |
