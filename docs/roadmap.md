...
| **H6**             | **In progress**        | Provider URLs use one fail-closed policy and runtime destination fallback URLs are removed. Foreground requests snapshot the approved scheme/host/effective port, allow same-origin redirects, and reject cross-origin/user-info redirects. Because iOS background sessions always follow redirects without delegate approval, new Whisper uploads now use the origin-locked foreground transport; the legacy background uploader only drains existing tasks. Response-size/deadline unification, a safe background design, HTTP-date rate limits, request identity, cooldown, and connection-cost policy remain. |
...
| **H6**  | Active cloud requests are pinned to the approved origin; background upload is disabled until a safe design exists, and response/deadline/rate-limit policy remains incomplete                      | Oversized responses, mismatched retry waits, or ambiguous replay can still hang work or repeat paid operations              | **P0**                |
...
5. `WhisperBackgroundUploader` is disabled for new work because its session
   cannot reject redirects. Restore background transfer only with an explicit safe
   design; then unify `Retry-After`, initialization, response caps, and persisted
   logical request reconciliation. Legacy tasks still drain through its delegate.
