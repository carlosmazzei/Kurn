...
| **H3**             | **PR 1 and PR 2 merged** | PR 1 ([#159](https://github.com/carlosmazzei/Kurn/pull/159)) fixed the risk register's deletion hazard: `RecordingTrash` makes meeting/recording deletion move-then-purge instead of delete-then-delete, with launch/foreground reconciliation for an interrupted delete. PR 2 ([#160](https://github.com/carlosmazzei/Kurn/pull/160)) fixes the JSON-corruption hazard for `Transcript.segments`/`Summary.sections` — see the H3 PR 2 handoff above the H4 plan. The rest of the track — general operation journal, quarantine for unmatched originals, migration collision handling, and the unprotected `recordingsDirectoryURL` fallback — is not yet started. |
...
| **H3**  | Deletion no longer removes audio before the SwiftData save commits (PR 1, merged); a corrupted transcript/summary is no longer indistinguishable from an empty one (PR 2, merged); recovery still deletes some unmatched files | A partial failure during a non-delete mutation or an unmatched original can still lose or misrepresent data | **P0**                |
