...
| **H6**             | **In progress**        | Provider URLs fail closed and runtime destination fallbacks are removed. All new cloud traffic uses one origin-locked foreground policy with a total operation deadline, remaining per-attempt timeout, 16 MB incremental response cap, cooperative cancellation, bounded retry/jitter, and typed transport/status/size errors. Background Whisper upload is disabled because iOS cannot reject its redirects; the legacy uploader only drains existing tasks. HTTP-date rate limits, a safe background design, request identity, cooldown, and connection-cost policy remain. |
...
| **H6**  | Active cloud requests are origin-pinned, deadline-bounded, and response-capped; background upload is disabled, while HTTP-date rate limits and request identity remain incomplete                  | A server rate-limit instruction or ambiguous response loss can still delay incorrectly or repeat paid operations            | **P0**                |
