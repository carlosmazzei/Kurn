...
| **H6**             | **In progress**        | Provider URLs fail closed and all new cloud traffic uses one origin-locked, deadline-bounded, 16 MB-capped foreground policy with exact budgeted `Retry-After`. Background upload is now a compile-time-disabled contract: its synchronized legacy session only reattaches system-owned tasks, queues every iOS completion handler, cancels orphan responses before buffering, and caps any awaited legacy body. Dedicated waiting UI, request identity, cooldown, and connection-cost policy remain.                                                              |
...
5. **Implemented as foreground-only (2026-08-30).** New background uploads are
   unavailable at compile time because iOS cannot reject their redirects. The
   legacy session is initialized under lock solely to drain old tasks, retains all
   relaunch completion handlers, rejects orphan response bodies before buffering,
   and enforces the foreground response cap for any still-awaited legacy task.
