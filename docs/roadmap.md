...
### H5 · Typed degradation and output-integrity gates — P1, next track
...
8. **Next: H5 (P1), typed stage outcomes and the persisted pipeline report**
   (`docs/resilience-megaplan.md`'s PR 11 boundary), then the final integrity
   gate and atomic artifact replacement (PR 12). Every stage returns
   requested/effective engine plus a `succeeded`/`degraded`/`skipped`/`failed`
   outcome with a stable reason; the aggregate report persists beside the
   transcript so "completed with warnings" is durable rather than inferred.
