...
| **H6**             | **In progress**        | Provider URLs now use one fail-closed policy in Settings, factories, JSON requests, transcription, and model listing: public HTTPS hostname, standard port, no credentials/query/fragment, and no local or IP-literal destination. Runtime destination fallback URLs are removed and invalid configuration has a typed error. Redirect/origin pinning, response-size/deadline policy, background uploads, HTTP-date rate limits, request identity, cooldown, and connection-cost policy remain.                                                                    |
...
| **H6**  | Request construction now fails closed on invalid destinations, but URLSession redirects are not pinned to the approved origin and foreground/background policy remains split                       | A redirect or background-policy gap can still move credentials/content outside the approved operation boundary              | **P0**                |
