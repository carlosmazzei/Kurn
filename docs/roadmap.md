...
| **H2**             | **Schema baseline CI-green (PR #155, awaiting review)** | `KurnSchemaV1`/`KurnSchemaMigrationPlan` (`Kurn/Infrastructure/KurnSchema.swift`) centralize the model graph for production, screenshots and tests, and `ModelContainerBootstrap` takes an injectable schema/migration plan; `LegacyStoreAdoptionTests` round-trips an unversioned-store fixture through it. GitHub `iOS CI` (`build-and-test`, `kurncore-linux`) passed on commit `e5b84eb`. Production still terminates with `fatalError`; the boot state machine, protected backups, failure classification, and recovery UI remain. See the H2 schema-baseline handoff above the H2 plan for what shipped and what CI observed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| **H2**  | A `VersionedSchema`/`SchemaMigrationPlan` now exists and is injectable, but production `ModelContainer` construction still ends in `fatalError` with no boot state machine or protected backup                                     | Corruption, incompatible schema, or a locked protected store can still create a launch crash loop until the boot state machine (H2 PR 3) lands | **P0**                |
...
#### H2 schema-baseline PR — CI green, awaiting review (2026-08-30)

[PR #155](https://github.com/carlosmazzei/Kurn/pull/155), branch
`claude/plano-resiliencia-xe25b2`, not yet merged. This session has no
macOS/Xcode toolchain, so — per "Verifying without a local macOS/Xcode
toolchain" above — nothing here was claimed to compile or pass locally before
CI ran; the GitHub `iOS CI` result on the PR is the source of truth, and it is
now green (see "Observed on PR #155" below). `mergeable_state` reports
`blocked` only on required human review/approval — there is no merge conflict
...
1. **H2 versioned schema baseline — CI green on [PR #155](https://github.com/carlosmazzei/Kurn/pull/155), awaiting review/merge.** The
   ...
   `build-and-test` and `kurncore-linux` both passed on commit `e5b84eb`; the
   PR is blocked only on required human review, not on CI or a merge conflict.
2. **Finish H2 (P0): recoverable bootstrap.** Replace production `fatalError` with
   ...
   salvage and explicitly confirmed fresh start. This is the next PR to
   start, from `main` after PR #155 merges.
