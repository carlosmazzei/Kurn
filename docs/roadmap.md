...
| **H5**             | **In progress (PR 11 merged in [#168](https://github.com/carlosmazzei/Kurn/pull/168); PR 12 implemented)** | Typed stage outcomes and a pipeline report persisted in `Transcript.pipelineReportData` are implemented for every stage (plan items 1–2 and the durable half of item 3, PR 11, merged). `TranscriptIntegrityGate` rejects a structurally broken fused/corrected result or an identity-violating correction before it can replace an existing transcript (items 4–5, PR 12); summary and semantic-index replacement already kept the previous artifact until the new one was ready and needed no change. The completed-with-warnings UI with stage-specific retry (PR 13) remains.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| **H5**  | `TranscriptIntegrityGate` now rejects a structurally broken fused/corrected result or a correction that violated its identity contract before it can replace an existing transcript (H5 PR 12); VAD/LID/diarization degradation is now recorded in `PipelineReport` rather than silently returning a normal-looking fallback (H5 PR 11)                                                                                                                | Resolved for the paths above; only the completed-with-warnings UI and stage-specific retry (PR 13) remain                                                    | **P1 → mostly closed** |
...
**Progress — items 4–5 (the PR 12 boundary), implemented on branch
`claude/resilience-roadmap-plan-fn23ki`.** `TranscriptIntegrityGate`
...
8. **In flight: H5 (P1).** Typed stage outcomes and the persisted pipeline
   ...
   atomic artifact replacement (PR 12) is implemented next: a structurally
   ...
   transcript. Surfacing the report in the UI is PR 13.
