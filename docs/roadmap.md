...
log sites (the PR 22 boundary), implemented on branch
`claude/resilience-roadmap-plan-fn23ki`.** Item 6's "keep the reliability
...
11. **In flight: H9 (P1/P2), PR 21 merged, PR 22 implemented.** `AppError` gains
    ...
    verbatim already is the redaction preview item 6 asks for — implemented
    on branch `claude/resilience-roadmap-plan-fn23ki`. Items 2, 4, 7–8
    remain PR 23's scope, plus the deferred UI reference-id and the
    broader non-`AppError` log-site sweep.
