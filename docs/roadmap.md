...
| **H6**             | **In progress**        | Provider URLs fail closed and all new cloud traffic uses one origin-locked, deadline-bounded, 16 MB-capped foreground policy. `Retry-After` honors delta-seconds and all HTTP-date forms exactly within explicit budgets: 30 seconds for interactive work and up to the 300-second operation deadline for transcription; a larger instruction fails visibly instead of being truncated. Waits remain cancellable and content-free in logs. Background upload stays disabled; dedicated waiting UI, a safe background design, request identity, cooldown, and connection-cost policy remain. |
...
| **H6**  | Active cloud requests are origin-pinned, bounded, and honor server rate limits within explicit budgets; background upload is disabled, while request identity/cooldown remain incomplete           | Ambiguous response loss or repeated automated activation can still repeat paid operations                                   | **P0**                |
