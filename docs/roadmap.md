...
| **H7**  | `KeychainAccessing` now classifies absent vs locked/denied/transient instead of collapsing every failure to "not configured", and the accessibility migration only completes after a confirmed outcome (H7 PR 14); app-managed models are still accepted by loose size bounds and replaced non-transactionally                                                                                                    | Resolved for the Keychain paths above; a model can still be missing, corrupt, or have an older valid copy destroyed until PR 15/16 land                     | **P1 → partially closed** |
...
### H7 · Credential and model integrity — P1, items 1–2 done
...
8. **H5 (P1) done.** Typed stage outcomes and the persisted pipeline
   ...
   plan.
9. **In flight: H7 (P1), items 1–2.** Typed Keychain outcomes
   (`docs/resilience-megaplan.md`'s PR 14 boundary) replace the old API that
   collapsed every Security-framework failure into "not configured": a
   `KeychainAccessing` seam, a fixed accessibility migration that only
   completes after a confirmed outcome, and explicit-Save for provider
   credentials (URL validated first, a failed Keychain write surfaced rather
   than silently assumed). Verified/resumable model staging and health
   probes (PR 15/16) remain.
