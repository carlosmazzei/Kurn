...
| **H4**             | **PR 8 open ([#165](https://github.com/carlosmazzei/Kurn/pull/165))** | `TranscriptionPipelineFingerprint` and `TranscriptionCheckpoint.isStructurallyValid` (items 1–2 of the plan) replace the old engine/language/compaction/provider-only match with source size/duration/content-digest, effective preprocessing/VAD, exact ASR provider+model, a compaction-map digest, and a separately-checked exact chunk-plan digest — see the "Progress" note under H4 in the roadmap for what shipped and the one deliberate compatibility break. Items 3–6 (throwing checkpoint commits, explicit operation states, bounded automatic recovery, and the same pattern for summary/wiki/document/correction jobs — the megaplan's PR 9/PR 10 boundaries) are not started.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| **H4**  | A checkpoint now fingerprints source content, preprocessing/VAD, exact ASR provider/model, the compaction map, and the exact chunk plan (PR 8); it still doesn't gate forward progress with a throwing durable commit or use explicit operation states | A checkpoint-save failure can still let the run continue past a chunk that was never actually made durable | **P0**                |
...
- **Items 3–6 (throwing checkpoint commits, explicit operation states,
  bounded automatic recovery, and the same durable-job pattern applied to
  summary/wiki/document/correction generation) — not started.** These are
  the megaplan's PR 9 and PR 10 boundaries.
