...
| **H1**             | **In progress**        | `RecordingSink` latches the first conversion/write/final-drain failure and exposes frame progress. A two-second monotonic watchdog now pauses capture when written frames stop advancing, attempts the existing engine recovery when it is no longer running, and offers Resume-to-retry or Stop-to-preserve without erasing the historical warning. `RecorderViewModel` preserves partial output and waits for warning dismissal before reporting completion. Storage runway, provisional rows, authoritative final-file validation, and the process-death/device matrix remain. |
...
| ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| **H1**  | Sink failures and a two-second written-frame watchdog pause live capture and preserve a warned partial result; elapsed duration is still wall-clock and there is no capacity preflight or final-file validation | Low storage is not predicted before capture and an unreadable or empty file can still pass finalization                     | **P0**                |
